...
if(isset($_REQUEST["dec"]) and $_REQUEST["dec"]=="1"){
  ...
  if(isset($_GET['id'])){
      if(($_GET['id'])){
          $qq = mysql_query("select * from offre where id = ".$_GET['id']);
  ...
  if(isset($_POST['titre']) and isset($_POST['desc']) and isset($_POST['id'])){
    if(($_POST['titre']) and ($_POST['desc']) and ($_POST['id'])){


      
        mysql_query("update offre set titre = '".
            addslashes(utf8_encode($_POST['titre']))."', descr = '".
            addslashes(utf8_encode($_POST['desc']))."' where id= ".$_POST['id']);
      

      
      echo "<script>document.location.href='editOffre.php?id=".$_POST['id']."';</script>";
    }
  }
?>
        ...
        <section class="content">
              ...
              <div class='box'>
                <div class='box-header'>
                  <h3 class='box-title'>Edition offre<small></small></h3>
                  <!-- tools box -->
                ...
                <div class='box-body pad'>
                  <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label>Titre:</label>
                        <input type="text" class="form-control" name="titre" value="<?php if(isset($tt) and $tt[2])echo stripslashes(utf8_decode($tt[1])); ?>" />
                    </div>
                    <div class="form-group">
                        <label>Description:</label>
                        
                        <textarea name="desc" class="textarea" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php if(isset($tt) and $tt[2])echo stripslashes(utf8_decode($tt[2])); ?></textarea>

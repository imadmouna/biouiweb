...
if(isset($_REQUEST["dec"]) and $_REQUEST["dec"]=="1"){
  ...
  if(isset($_GET['spr'])){
    if(($_GET['spr'])){
      mysql_query("delete from classification where id=".$_GET["spr"]);
      echo "<script>alert('Suppression valide !');</script>";
      echo "<script>document.location.href='classification.php';</script>";
    }
  }


  if(isset($_GET['texte']) and $_GET['texte']){
      mysql_query("update texte_classification set texte='".addslashes(utf8_encode($_GET['texte']))."'");
      echo "<script>document.location.href='classification.php';</script>";
  }
  ...
  if(isset($_POST['desc']) and isset($_FILES['photo']['tmp_name'])){
    if(($_POST['desc']) and ($_FILES['photo']['tmp_name'])){
      ...
      mysql_query("insert into classification values(null, '".
...
<html>
  ...
  <body class="skin-blue">
                      ...
                      <tr>
                          ...
                          <td>
                              <div class="col-md-6 pull-right">
                                  <div class="col-md-6">
                                      <a href="editCertif.php?id=<?php echo $tqq[0];?>&cat=classification" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Modifier<a/>
                                  </div>
                                  <div class="col-md-6">
    ...
    $(".btn-spr").click(function(e){
        ...
        $(".btn-spr-oui").attr("href", "classification.php?spr="+id);
